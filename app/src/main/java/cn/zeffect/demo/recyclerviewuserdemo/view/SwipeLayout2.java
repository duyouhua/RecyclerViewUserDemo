package cn.zeffect.demo.recyclerviewuserdemo.view;
...
public class SwipeLayout2 extends LinearLayout {
    ...
    static List<SwipeLayout2> swipelayouts = new ArrayList<>();
        public  static void addSwipeView(SwipeLayout2 v){
            if(null==v){
                return;
            }
            swipelayouts.add(v);
        }
        public static void removeSwipeView(SwipeLayout2 v){
            if(null==v){
                return;
            }
            v.SimulateScroll(SwipeLayout2.SHRINK);
        }
        private void shrinkAllView(){
            for(SwipeLayout2 s :swipelayouts){
                if(null==s){
                    swipelayouts.remove(s);
                    continue;
                }else {
                    s.SimulateScroll(SwipeLayout2.SHRINK);
                }
